...
(function( window, $, _, Backbone, wp, undefined ) {
	...
	Control = function( el, options ) {
		...
		selector = this.$el.data( 'target' ) || '.os_image_widget-control-target';
	...
	_.extend( SimpleImageWidget, {
		...
		getControl: function( el ) {
			...
			$control = $( el ).closest( '.os_image_widget-control' );
	...
	finder.view.Page = wp.Backbone.View.extend({
		events: {
			'click .os_image_widget-find-posts-button' : 'openModal'
		},
		...
		openModal: function( e ) {
			var $target = $( e.target).closest( '.os_image_widget-input-group' ).find( 'input[type="text"]' );
			e.preventDefault();
	...
	finder.view.Modal = wp.Backbone.View.extend({
		tagName: 'div',
		className: 'find-box',
		template: wp.template( 'os_image_widget-modal' ),

		events : {
			'click .os_image_widget-modal-search-button': 'findPosts',
			'keypress .os_image_widget-modal-search-field': 'routeKey',
			'keyup .os_image_widget-modal-search-field': 'routeKey',
			'click tr': 'select',
		...
		render: function() {
			...
			this.$field = this.$el.find( '.os_image_widget-modal-search-field' );
			this.$overlay = $( '.ui-find-overlay' );
			this.$response = this.$el.find( '.os_image_widget-modal-response' );
			this.$spinner = this.$el.find( '.os_image_widget-modal-search .spinner' );
			return this;
		},
		...
		findPosts: function() {
			...
			wp.ajax.post( 'OS_Image_Widget_find_posts', {
				s: self.$field.val(),
	...
	$( document ).ready(function() {
		...
		$body.on( 'click', '.os_image_widget-control-choose, .os_image_widget-form img', function( e ) {
			e.preventDefault();
		...
		$body.on( 'selectionChange.simpleimagewidget', '.os_image_widget-control', function( e, selection ) {
			var $control = $( e.target ),
				$sizeField = $control.closest( '.os_image_widget-form' ).find( 'select.image-size' ),
			...
			$control.find( 'img' ).remove().end()
				.prepend( image )
				.addClass( 'has-image' )
				.find( 'a.os_image_widget-control-choose' ).removeClass( 'button-hero' );
		});

		// Wire up the toggle checkboxes in the screen options tab.
		$( '.os_image_widget-field-toggle' ).on( 'click', function() {
			var $this = $( this ),
				field = $this.val(),
				$hiddenFields = $( '.os_image_widget-field-toggle:not(:checked)' );

			$( '.os_image_widget-field-' + field ).toggleClass( 'is-hidden', ! $this.prop( 'checked' ) );

			$.ajax({
				url: ajaxurl,
				type: 'POST',
				data: {
					action: 'OS_Image_Widget_preferences',
		...
		$body.on( 'focus', '.os_image_widget-input-group-field', function() {
			$( this ).parent().addClass( 'is-focused' );
		}).on( 'blur', '.os_image_widget-input-group-field', function() {
