 ...
 * @since   3.0.0
 */
class OS_Image_Widget extends WP_Widget {
	/**
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		...
		$widget_options = wp_parse_args(
			...
			array(
				...
				'description'                 => __( 'An image with text and link.'),
			)
		);
	...
	public function widget( $args, $instance ) {
		$cache    = (array) wp_cache_get( 'OS_Image_Widget', 'widget' );
		...
		if ( ! empty( $instance['image_id'] ) ) {
			...
			if ( ! $image || 'attachment' != get_post_type( $image ) ) {
				$output = '<!-- OS Image Widget Error: Invalid Attachment ID -->';
			}
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'OS_Image_Widget', array_filter( $cache ), 'widget' );
		}
	}
	...
	public function render( $args, $instance ) {
		 ...
		 */
		$inside = apply_filters( 'OS_Image_Widget_output', '', $args, $instance, $this->id_base );
		...
		if ( $inside ) {
			...
			$data['image_size'] = $image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'OS_Image_Widget_output_default_size', 'medium', $this->id_base );
			$data['title'] = ( empty( $instance['title'] ) ) ? '' : $instance['title'];
			$data = array_merge( $instance, $data );
			$data = apply_filters( 'OS_Image_Widget_template_data', $data );

			ob_start();
			$templates = $this->get_template_names( $args, $instance );

			$template_loader = new OS_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'os_image_widget-control-choose' );
		 ...
		 * @param string $id_base Widget type id.
		 */
		$fields = (array) apply_filters( 'OS_Image_Widget_fields', $this->form_fields(), $this->id_base );
		?>

		<div class="os_image_widget-form">
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'OS_Image_Widget_form_before', $instance, $this->id_base );
			?>
			...
			<?php if ( ! is_OS_Image_Widget_legacy() ) : ?>
				<p class="os_image_widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Select Image'); ?>"
					data-update-text="<?php esc_attr_e( 'Select Image'); ?>"
					...
					?>
					<input type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ); ?>" value="<?php echo absint( $image_id ); ?>" class="image-id os_image_widget-control-target">
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Select Image'); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						/*case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size'); ?>:</label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text'); ?>:</label>
						...
						default :
							 ...
							 * registered with the 'OS_Image_Widget_fields'
							 * filter.
							 *
							 * @since 3.0.0
							 *
							 ...
							 */
							do_action( 'OS_Image_Widget_field-' . sanitize_key( $field ), $instance, $this );
					}
				}
			}
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'OS_Image_Widget_form_after', $instance, $this->id_base );
			?>

		</div><!-- /.os_image_widget-form -->
		<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance = apply_filters( 'OS_Image_Widget_instance', $instance, $new_instance, $old_instance, $this->id_base );
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size') );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = OS_Image_Widget_Plugin::get_image_size_names();
	...
	public function flush_group_cache( $post_id = null ) {
		...
		wp_cache_delete( 'OS_Image_Widget', 'widget' );
	}
	...
	protected function siw_field_class( $id ) {
		$classes = array( 'os_image_widget-field', 'os_image_widget-field-' . sanitize_html_class( $id ) );

		$hidden_fields = OS_Image_Widget_Plugin::get_hidden_fields();
