...
/*
    ...
    Version: 1.0
    Partner: Roberto Moreno
    Partner Email: rmoreno@opensistemas.com
    Plugin URI: http://www.opensistemas.com/
   ...
   class PartnerCustomType {   
        ...
        function create_post_type() {
            register_post_type($this->post_type, array(
                 'labels' => array(
                    'name' => __('Partner', 'os_equipo_Partner_type'),
                    'singular_name' => __('Partner', 'os_equipo_Partner_type'),
                    'add_new' => __('Añadir nuevo Partner', 'os_equipo_Partner_type'),
                    'add_new_item' => __('Añadir nuevo Partner', 'os_equipo_Partner_type'),
                    'new_item' => __('Nuevo Partner', 'os_equipo_Partner_type'),
                    'edit_item' => __('Editar Partner', 'os_equipo_Partner_type'),
                    'view_item' => __('Ver Partner', 'os_equipo_Partner_type'),
                    'parent_item_colon' => __(' ', 'os_equipo_Partner_type'),
                    'search_items' => __('Buscar Partner', 'os_equipo_Partner_type'),
                    'not_found' => __('Partner no encontrado', 'os_equipo_Partner_type'),
                    'not_found_in_trash' => __('Partner no encontrado en la papelera', 'os_equipo_Partner_type'),
                    'menu_name' => __('Partner', 'os_equipo_Partner_type')
                ),
        ...
        function meta_boxes_add() {
            ...
            add_meta_box("data" ,__("Partner data", "os_partner_type"),array(&$this, "Partner_data_meta_box_callback"),$this->post_type);
            add_meta_box("image" ,__("Image Partner", "os_partner_type"),array(&$this, "image_meta_box_callback"),$this->post_type);            
        }


        function Partner_data_meta_box_callback($post) {

            wp_nonce_field( basename(__FILE__), 'Partner-nonce');         
        ...
        function register_admin_styles(){
            ...
            if ($typenow == $this->post_type) {
                wp_enqueue_style('Partner-type-css', plugin_dir_url( __FILE__ ) . 'css/os_partner_type.css');               
            }
        }
        ...
        function register_admin_scripts() {
            ...
            if ($typenow == $this->post_type) {
                wp_enqueue_media();
                wp_register_script('ly-Partner-type-js', plugins_url('js/os_partner_type.js' , __FILE__), array('jquery'));
                wp_enqueue_script('ly-Partner-type-js');
            }
        }

    }

    $lyPartner = new PartnerCustomType();
